...
assert(isequal(tr.flip,[0 0 0]));      % currently, the GUI does not allow this to be edited
assert(isequal(tr.reorder,[1 2 3]));   % currently, the GUI does not allow this to be edited
fun = @(pp) feval(metricfun, ...
  subscript(reslicevolume(0,maketransformation(tr.flip,tr.reorder,pp(1:3),tr.rotorder,pp(4:6), ...
